...
class Lost_Book_Class:
    ...
    def __init__(self, username, dbpassword):
        ...
        def Book_Display_Data(event=""):
            ...
            cur.execute("SELECT `book_id`,`book_name`, `book_sub`, `author`,`lost_date` FROM `books_details` NATURAL JOIN lost_book;")
            result = cur.fetchall()
            ...
            try:
                if self.P_Search.get() == "":
                    ...
                    if self.Pcmb.get() == "Book Name":
                        ...
                        cur.execute("SELECT `book_id`,`book_name`, `book_sub`, `author`,`lost_date` FROM `books_details` NATURAL JOIN lost_book where book_name like '%" +
                                    Nam[0]+"%'order by id_counter;")
                    elif self.Pcmb.get() == "Book Id":
                        cur.execute("SELECT `book_id`,`book_name`, `book_sub`, `author`,`lost_date` FROM `books_details` NATURAL JOIN lost_book where book_id =%s;",(self.P_Search.get()))  
        ...
        top.title("Library Management System | Lost Books Records")
        ...
        Search_Book_Inner_Frame = LabelFrame(Search_Book_Frame, text="Search Record", fg="green", bd=10, relief=RIDGE,
        ...
        Input_P_cmb_search = ttk.Combobox(Search_Book_Inner_Frame, textvariable=self.Pcmb, values=(
            "Book Name","Book Id"), cursor="hand2", state="readonly", justify=CENTER, font=("Comic Sans MS", 16, "bold"))
        ...
        self.Book_table = ttk.Treeview(Book_Table_Frame, column=("bid","bn", "bs", "au", "lod"), xscrollcommand=Book_Xscroll.set, yscrollcommand=Book_Yscroll.set)
        ...
        self.Book_table.heading("bid", text="Book ID")
        ...
        self.Book_table.heading("lod", text="Lost Date")
        ...
        self.Book_table.column("bid", width=100, stretch=0, anchor=CENTER)
        self.Book_table.column(
            "lod", width=150, stretch=0, anchor=CENTER)
